  ...
  >
    ...
    <div class="mt-3 flex items-center justify-between">
      <span class="text-[11px] text-text-muted"
        >Created at: {{ formatDateAndTime(props.task.created_at) }}</span
      >
      <div v-if="props.canEdit" class="flex flex-row gap-1">
        <button
        ...
        <button
          class="w-6 h-6 rounded-lg cursor-pointer flex items-center justify-center text-text-danger/60 hover:bg-bg-danger-hover hover:text-text-danger/80 transition-all duration-150"
          title="Delete"
